...
#define SNDSB_BIO_DSP_RESET                                     0x0600
#define SNDSB_BIO_DSP_READ_DATA                                 0x0A00
#define SNDSB_BIO_DSP_WRITE_DATA                                0x0C00
#define SNDSB_BIO_DSP_WRITE_STATUS								0x0C00
#define SNDSB_BIO_DSP_READ_STATUS                               0x0E00
#else
...
{
  //Reset the DSP
  outp (Test + SNDSB_BIO_DSP_RESET, 1);
  delay(10);
  outp (Test + SNDSB_BIO_DSP_RESET, 0);
  delay(10);
  //Check if (reset was succesfull
  if (((inp(Test + SNDSB_BIO_DSP_READ_STATUS) & 0x80) == 0x80) && (inp(Test + SNDSB_BIO_DSP_READ_DATA) == 0xAA))
  {
    //DSP was found
    Base = Test;
    return (1);
...
static void WriteDSP(unsigned char Value) {
  int hibaw = 0;
  while ((inp(Base + SNDSB_BIO_DSP_WRITE_STATUS) & 0x80) == 0x80) {
    if (++hibaw == 512) return;
  }
  outp(Base + SNDSB_BIO_DSP_WRITE_DATA, Value);
}
...
{
  //Relieve DSP
  inp (Base + SNDSB_BIO_DSP_READ_STATUS);
...
static void SBDMA_Init()
{
	...
	if (ResetDSP (BASE_ADDR)) {
		printf ("Sound Blaster found at %Xh\n", Base);
	}
	else
	{
		printf("NOT FOUND SB16\n");
	}
	//else 
	...
	outp (Base + SB_MIXER_ADDRESS, 0x22); // Set IRQ mask
	outp (Base + SB_MIXER_DATA, 1 << (IRQ - 2));
	outp (Base + SB_MIXER_ADDRESS, 0x81); // Set DMA mask
	outp (Base + SB_MIXER_DATA, 1 << DMA);
